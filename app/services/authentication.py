...
async def validate_token(token: str = Depends(OAuth2PasswordBearer(tokenUrl='token'))) -> bool:
    """
    This method is used to validate that the given token is""" +\
        """ correct(not modified) and the stakeholder is/still authorized
    """
...
async def current_stakeholder(token: str = Depends(
        OAuth2PasswordBearer(tokenUrl='token'))) -> StakeholderDataResponse:
    """
    This method is used to return the current authorized""" +\
        """ stakeholder information using the token as a input.
    """
    credentials_exception = HTTPException(
        status_code=status.HTTP_401_UNAUTHORIZED,
        detail='Could not validate credentials',
...
async def validate_admin(current_stackholder: StakeholderDataResponse =
                         Depends(current_stakeholder)) -> bool:
    """
...
async def current_admin_email(current_stakeholder: StakeholderDataResponse =
                              Depends(current_stakeholder)) -> str:
    """
