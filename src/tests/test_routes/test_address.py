...
def test_get_all_addresses(mocker: MockerFixture, client: TestClient,
    ...
    mock_connect = mocker.patch('src.routes.address.engine.connect')
...
def test_create_address(mocker: MockerFixture, client: TestClient,
    ...
    mock_connect = mocker.patch('src.routes.address.engine.begin')
    mock_connect.return_value.__enter__.return_value.execute.return_value.first.return_value = mock_response_address_data
...
def test_update_address(mocker: MockerFixture, client: TestClient,
    ...
    if found:
        ...
        mock_connect.return_value.__enter__.return_value.execute.return_value.first.return_value.rowcount = 0
    with pytest.raises(AttributeError):
        with client.patch(f'/addresses/{mock_address_id}', json=mock_response_address_data) as response:
            assert mock_connect.called
            assert response.status_code == status_code
            assert response.json() == response_data
